...
Os pacotes do STCP Gemini Server são disponibilizados nos repositórios oficiais da Riversoft. Para facilitar a configuração do ambiente, são fornecidos arquivos `.repo` específicos para cada versão do Enterprise Linux suportada.
...
Os arquivos disponibilizados pela Riversoft configuram automaticamente os repositórios de produção correspondentes à arquitetura do servidor (`$basearch`) e aos pacotes independentes de arquitetura (`noarch`).
...
Todos os repositórios utilizam validação de assinatura GPG (`gpgcheck=1`). A chave pública utilizada na validação dos pacotes é obtida em:
...
> [!IMPORTANT]
> **Importante:** Caso já exista uma configuração anterior do repositório Riversoft em `/etc/yum.repos.d/`, verifique e remova ou desabilite arquivos antigos antes de aplicar a nova configuração, evitando repositórios duplicados ou conflitos durante a instalação e atualização dos pacotes.

Após configurar o repositório, atualize os metadados do gerenciador de pacotes antes de prosseguir com a instalação:
