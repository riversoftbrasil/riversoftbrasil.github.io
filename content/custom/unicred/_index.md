---
linkTitle: STCP OFTP Client | Unicred
title: 
excludeSearch: true
layout: docs
toc: true
...
| Porta                 | **Fornecido pela {{< param cliente >}}**  |   
...
```bash
$ stcpclient -p O0055UNICRED -r 1 /usr/local/stcpclient/configs/stcpclient.config.json
```
...
```
/usr/local/stcpclient/data/STCPClt/O0055UNICRED/SAIDA
```
...
```
/usr/local/stcpclient/data/STCPClt/O0055UNICRED/ENTRADA
```
...
0 0 * * * /bin/bash -l -c "PATH=\"$PATH:/usr/local/bin\"; stcpclient -p O0055UNICRED -r 1 /usr/local/stcpclient/configs/stcpclient.config.json"
```
